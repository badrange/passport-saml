...
SAML.prototype.requestToUrl = function (request, response, operation, additionalParameters, callback, finalQueryParams) {
  var self = this;
  ...
  function requestToUrlHelper(err, buffer) {
    ...
    // Apply final query params.
    Object.keys(finalQueryParams).forEach(function(k) {
        target.query[k] = finalQueryParams[k];
      });
...
SAML.prototype.getAuthorizeUrl = function (req, options, callback, finalQueryParams) {
  var self = this;
  self.generateAuthorizeRequest(req, self.options.passive, function(err, request){
    ...
    self.requestToUrl(request, null, operation, self.getAdditionalParams(req, operation, overrideParams), callback, finalQueryParams);
  });
};
